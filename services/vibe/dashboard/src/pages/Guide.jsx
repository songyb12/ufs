...
const sections = [
  ...
  },
  {
    ...
    content: [
      {
        heading: '백테스트란?',
        text: '백테스트는 VIBE 파이프라인의 시그널(BUY/SELL/HOLD)을 과거 시장 데이터에 적용하여, 실제로 그 시그널대로 매매했을 때 어떤 성과가 나왔는지 시뮬레이션하는 기능입니다. 전략의 유효성을 과거 데이터로 검증하므로, 실전 투자 전에 전략 신뢰도를 객관적으로 평가할 수 있습니다.',
      },
      {
        heading: '백테스트 동작 원리',
        items: [
          '1. 지정된 기간의 과거 가격/지표 데이터를 조회합니다',
          '2. 매일 VIBE 파이프라인 로직(기술적 분석 → 매크로 → Hard Limit → 시그널)을 재실행합니다',
          '3. BUY 시그널 → 가상 매수, SELL 시그널 → 가상 매도를 수행합니다',
          '4. 모든 가상 매매의 수익률, 적중률, 샤프 비율 등 성과 지표를 산출합니다',
        ],
      },
      {
        heading: '주요 성과 지표 설명',
        items: [
          ...
          'Sharpe Ratio (샤프 비율) — 위험 대비 수익. 1.0 이상이면 양호, 2.0 이상이면 우수',
          'Max Drawdown (최대 낙폭) — 최고점 대비 최대 하락폭. -10% 이내가 이상적',
          'Profit Factor — 총 수익 / 총 손실. 1.5 이상이면 양호',
          'Win/Loss Ratio — 평균 수익 / 평균 손실 비율',
      ...
      },
      {
        heading: '서머리 카드',
        items: [
          'Total Runs — 지금까지 실행한 백테스트 총 횟수',
          'Best Hit Rate — 전체 백테스트 중 가장 높은 적중률',
          'Avg Sharpe — 모든 백테스트의 평균 샤프 비율',
        ],
      },
      {
        heading: '결과 테이블 & 트레이드 상세',
        items: [
          '행 클릭 — 해당 백테스트의 개별 트레이드 목록이 확장됩니다',
          '각 트레이드: 종목, 진입일/가격, 청산일/가격, 수익률, 보유일수, 청산사유 확인',
          'Exit Reason 종류: signal_sell(매도 시그널), stop_loss(손절), take_profit(익절), period_end(기간 종료)',
        ],
      },
      {
        heading: '실행 방법',
        items: [
          ...
          '기본 기간: 최근 60영업일 (약 3개월). DB에 저장된 가격 데이터 범위 내에서 실행',
          '실행 완료 후 결과 테이블에 자동 추가되며, 행 클릭으로 트레이드 상세 확인',
          '같은 마켓을 여러 번 실행하면 각각 독립된 결과로 저장 (비교 분석 가능)',
        ],
      },
      {
        heading: '주의사항',
        items: [
          ...
          '파이프라인이 한 번도 실행되지 않았다면 먼저 시스템 탭에서 파이프라인을 실행하세요',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    content: [
      ...
      {
        heading: '모델별 추천 가이드',
        text: 'VIBE의 각 LLM 기능은 요구하는 추론 깊이와 출력 형태가 다릅니다. 아래는 기능별 최적 모델 추천입니다.',
      },
      {
        heading: 'Claude 3.5 Haiku (추천 기본)',
        items: [
          '가격 — Input $0.80/MTok, Output $4.00/MTok (최저가)',
          '속도 — 응답 ~0.5초, 가장 빠름',
          'S7 Red-Team — 적합. JSON 구조화 응답 + 위험 요소 식별에 충분',
          'S8 한국어 해설 — 적합. 2~3문장 해설에 과도한 모델 불필요',
          'S9 시나리오 — 적합. 규칙 기반 + 간단한 보완 해설',
          '시황 브리핑 — 적합. 3~5문장 요약에 최적',
          '결론 — 월 ~$1.27로 전 기능 커버. 비용 대비 효율 최고',
        ],
      },
      {
        heading: 'Claude 3.5 Sonnet',
        items: [
          '가격 — Input $3.00/MTok, Output $15.00/MTok (약 4배)',
          '속도 — 응답 ~1~2초',
          '추천 용도 — 복잡한 시나리오 분석(S9)에서 정밀도 향상 필요 시',
          '비추천 — S7/S8에는 과도. Haiku 대비 품질 차이 미미',
          '예상 월 비용 — ~$5/월 (전 기능 활성화)',
        ],
      },
      {
        heading: 'Claude Opus 4',
        items: [
          '가격 — Input $15.00/MTok, Output $75.00/MTok (약 19배)',
          '속도 — 응답 ~3~5초',
          '추천 용도 — 월간 리포트, 심층 분석 등 1회성 고품질 작업',
          '비추천 — 일일 파이프라인에는 비용 비효율적',
          '예상 월 비용 — ~$25+/월 (비실용적)',
        ],
      },
      {
        heading: 'OpenAI GPT-4o-mini',
        items: [
          '가격 — Input $0.15/MTok, Output $0.60/MTok (최저가)',
          '속도 — 응답 ~0.5초',
          '한국어 품질 — Claude 대비 다소 열위. 전문 용어 정확도 낮음',
          '설정 — LLM_PROVIDER=openai, LLM_MODEL=gpt-4o-mini',
          '결론 — 비용 최소화 목적이면 고려. 한국어 품질 트레이드오프',
        ],
      },
      {
        heading: '최종 추천',
        items: [
          '기본 설정 — Claude 3.5 Haiku (전 기능 활성화, 월 ~$1.27)',
          'S9만 업그레이드 — LLM_SCENARIO_MODEL에만 Sonnet 지정 (향후 지원)',
          '비용 0원 운영 — 모든 LLM 기능 OFF. 규칙 기반만으로 충분히 동작',
          '주의 — 모델 변경 시 .env의 LLM_MODEL 수정 + Docker 재시작',
        ],
      },
    ],
  },
